...
public class EvpnManager implements EvpnService {
    ...
    public void onBgpEvpnRouteUpdate(EvpnRoute route) {
        ...
        log.debug(BGP_EVPN_ROUTE_UPDATE_START, route);
    ...
    public void onBgpEvpnRouteDelete(EvpnRoute route) {
        ...
        log.debug(BGP_EVPN_ROUTE_DELETE_START, route);
    ...
    private void addArpFlows(DeviceId deviceId,
        ...
        if (type.equals(Objective.Operation.ADD)) {
            log.debug(ROUTE_ADD_ARP_RULES);
            flowObjectiveService.forward(deviceId, objective.add());
        } else {
            log.debug(ROUTE_REMOVE_ARP_RULES);
     ...
     */
    private static Collection<PortNumber> getLocalTunnelPorts(Iterable<Port>
                                                                      ports) {
    ...
    private void setFlows(DeviceId deviceId, Host host, Label label,
                          ...
                          Operation type) {
        log.debug("Set the flows to OVS");
        ...
        Map<RouteTableType, Collection<Route>> allRoutes = routeService
                .getAllRoutes();
        if (allRoutes == null) {
            return;
        }

        Collection<Route> routes = allRoutes.get(RouteTableType.EVPN_IPV4);
        for (Route route : routes) {
            if (route instanceof EvpnRoute) {
                ...
                if (!hostByMac.isEmpty()
                        ...
                        .exportRouteTarget())))) {
                    log.debug("Route target import/export is not matched");
                    continue;
                }
                log.debug("Set the ARP flows");
                addArpFlows(deviceId, evpnRouteTem, type, host);
                ForwardingObjective.Builder build = getMplsOutBuilder(deviceId,
                                                                      evpnRouteTem,
                                                                      host);
                log.debug("Set the MPLS  flows");
                if (type.equals(Objective.Operation.ADD)) {
    ...
    public void onHostDetected(Host host) {
        log.debug(HOST_DETECT, host);
        DeviceId deviceId = host.location().deviceId();
        if (!mastershipService.isLocalMaster(deviceId)) {
            log.debug(NOT_MASTER_FOR_SPECIFIC_DEVICE);
            return;
        }
        ...
        if (!vpnPortService.exists(vpnPortId)) {
            log.debug(CANT_FIND_VPN_PORT, ifaceId);
            return;
        }
        ...
        if (!vpnInstanceService.exists(vpnInstanceId)) {
            log.debug(CANT_FIND_VPN_INSTANCE, vpnInstanceId);
            return;
        }
    ...
    private Label applyLabel() {
        ...
        log.debug(GET_PRIVATE_LABEL, privateLabel);
        return privateLabel;
    }

    @Override
    public void onHostVanished(Host host) {
        log.debug(HOST_VANISHED, host);
        DeviceId deviceId = host.location().deviceId();
        if (!mastershipService.isLocalMaster(deviceId)) {
            return;
        }
    ...
    private class InternalNetworkConfigListener implements
        ...
        public void event(NetworkConfigEvent event) {
            String subject;
            log.debug(NETWORK_CONFIG_EVENT_IS_RECEIVED, event.type());
            if (!event.configClass().equals(GluonConfig.class)) {
                return;
            }
            log.debug("Event is received from network configuration {}", event
                    .type());
            switch (event.type()) {
                case CONFIG_UPDATED:
            ...
            if (route instanceof EvpnInstanceRoute) {
                ...
                if (evpnInstanceRoute.evpnInstanceName().equals(evpnInstanceName)) {
                    ...
                    if (action.equals(UPDATE)) {
                        routeAdminService.update(Sets.newHashSet(evpnInstanceRoute));
                    } else if (action.equals(DELETE)) {
                        routeAdminService
                                .withdraw(Sets.newHashSet(evpnInstanceRoute));
                    }
                    ...
                    Collection<Route> evpnPublicRoutes
                            = routeService.getRoutesForNextHop(RouteTableType
                                                                       .EVPN_IPV4,
                                                               evpnNextHop);
                    for (Route pubRoute : evpnPublicRoutes) {
                        if (pubRoute instanceof EvpnRoute) {
                            EvpnRoute evpnPubRoute = (EvpnRoute) pubRoute;
                            if (evpnPubRoute.label().equals(evpnInstanceRoute
                                ...
                                evpnPubRoute
                                        .setImportRtList(new LinkedList<>(importRouteTargets));
                                if (action.equals(UPDATE)) {
                                    routeAdminService.update(Sets.newHashSet(evpnPubRoute));
                                } else if (action.equals(DELETE)) {
                                    routeAdminService
                    ...
                    if (action.equals(UPDATE)) {
                        ...
                        switch (routeTargetType) {
                            ...
                            case BOTH:
                                ...
                                break;
                            default:
                                log.debug(INVALID_ROUTE_TARGET_TYPE);
                                break;
                        }
                    }
