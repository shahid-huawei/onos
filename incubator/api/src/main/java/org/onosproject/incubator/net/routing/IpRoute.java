 ...
 * Represents a route.
 */
public class IpRoute implements Route {
     ...
     */
    public IpRoute(Source source, IpPrefix prefix, IpAddress nextHop) {
        checkNotNull(prefix);
        checkNotNull(nextHop);
        checkArgument(prefix.version().equals(nextHop.version()),
                      VERSION_MISMATCH);
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof IpRoute)) {
            return false;
        }

        IpRoute that = (IpRoute) other;
